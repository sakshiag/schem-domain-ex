...
data "ibm_infra_image_template" "base_template" {
  name = "${var.base_template_image}"
}
data "ibm_infra_image_template" "compute_template" {
  name = "${var.compute_node_image}"
}

resource "ibm_infra_virtual_guest" "domaincontroller" {
  count = "1"
  hostname = "${var.dc_hostname}"
  domain = "${var.domain}"
  image_id = "${data.ibm_infra_image_template.base_template.id}"
...
resource "ibm_infra_virtual_guest" "computenodes" {
  count = "${var.computenode_count}"
  hostname = "${var.cn_hostname}${count.index}"
  domain = "${var.domain}"
  image_id = "${data.ibm_infra_image_template.compute_template.id}"
  datacenter = "${var.datacenter}"
  cores = 2
  memory = 2048
  network_speed = 1000
  ...
  user_metadata = <<EOF
    ...
    $private_nic | Set-DnsClientServerAddress -ServerAddresses ("${ibm_infra_virtual_guest.domaincontroller.ipv4_address_private}")
    Sleep -Seconds 5
